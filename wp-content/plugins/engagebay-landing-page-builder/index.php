<?php
/*
Plugin Name: EngageBay Landing Page Builder
Plugin URI: https://www.engagebay.com
Description: EngageBay Landing Page Builder is the simplest and quickest way to build simple, stylish and responsive forms. Capture leads, grow email lists and dramatically improve conversions using our forms.
Version: 1.0
Author: EngageBay
Tested up to: 5.4.1
...
function engagebay_lp_builder_add_new_menu_items()
{
    add_menu_page('EngageBay Landing Pages', 'EngageBay Landing Pages', 'administrator', 'engage-bay-lp-builder', 'engagebay_lp_builder_options_page', plugins_url('engagebay-landing-page-builder/images/icon.png'), 0);
}

// Register style sheet.
add_action('wp_enqueue_styles', 'engagebay_lp_builder_css');
/**
 * Register style sheet.
 */
function engagebay_lp_builder_custom_js()
{
    wp_enqueue_script('custom_script', plugins_url('/js/pubnub.js', __FILE__), array('jquery'));
}

function engagebay_lp_builder_css()
{
    wp_enqueue_style('engagebay-forms', plugins_url('/css/style.css', __FILE__));
}

add_action('wp', 'engagebay_lp_builder_landing_page_setpup', 5, 0);
function engagebay_lp_builder_landing_page_setpup()
{
    if (!is_admin()) {
        global $post;
        $landing_page = get_post_meta($post->ID, 'engagebay_lp_builder_landing_page', true);
        if ($landing_page != '') {
            $domain = (sanitize_text_field(get_option('engagebay_lp_builder_domain')));
            $email = (sanitize_email(get_option('engagebay_lp_builder_email')));
            $rest_api = (sanitize_text_field(get_option('engagebay_lp_builder_rest_api')));
            if ($domain != '' && $email != '' && $rest_api != '') {
                $request = wp_remote_get('https://app.engagebay.com/landingpage/'.$landing_page);
...
add_action( 'plugins_loaded', 'engagebay_lp_builder_plugin_override' );
function engagebay_lp_builder_plugin_override() {
  if(isset($_GET['code'])) {
    ...
    try {    
      ...
      if($result && $result->rest_API_Key) {
        if (get_option('engagebay_lp_builder_rest_api') !== false) {
          update_option('engagebay_lp_builder_rest_api', $result->rest_API_Key);
        } else {
            add_option('engagebay_lp_builder_rest_api', $result->rest_API_Key);
        }
  
        $domain = explode(".",$result->version_url)[0];
        $domain = str_replace("https://", "", $domain);
  
        if (get_option('engagebay_lp_builder_domain') !== false) {
            update_option('engagebay_lp_builder_domain', $domain);
        } else {
            add_option('engagebay_domain', $domain);
        }
  
        if (get_option('engagebay_lp_builder_js_api') !== false) {
            update_option('engagebay_lp_builder_js_api', $result->js_API_Key);
        } else {
            add_option('engagebay_lp_builder_js_api', $result->js_API_Key);
        }
  
        if (get_option('engagebay_lp_builder_email') !== false) {
            update_option('engagebay_lp_builder_email', $user['email']);
        } else {
            add_option('engagebay_lp_builder_email', $user['email']);
        }
      }
...
function engagebay_lp_builder_options_page()
{
...
<?php
    ...
    if (isset($_GET['tab'])) {
        ...
        <?php $rest_api = (sanitize_text_field(get_option('engagebay_lp_builder_rest_api')));
    if ($rest_api) {
        ?>
        <!-- wordpress provides the styling for tabs. -->
        <h2 class="nav-tab-wrapper">
            <!-- when tab buttons are clicked we jump back to the same page but with a new parameter that represents the clicked tab. accordingly we make it active -->
            <!--<a href="?page=engage-bay-lp-builder&tab=home" class="nav-tab <?php if ($active_tab == 'home') {
            echo 'nav-tab-active';
        } ?> "><?php _e('Home', 'sandbox'); ?></a>-->

            <a href="?page=engage-bay-lp-builder&tab=web-pop" class="nav-tab <?php if ($active_tab == 'web-pop') {
            echo 'nav-tab-active';
        } ?>"><?php _e('Popup Forms', 'sandbox'); ?></a>
            <a href="?page=engage-bay-lp-builder&tab=forms" class="nav-tab <?php if ($active_tab == 'forms') {
            echo 'nav-tab-active';
        } ?>"><?php _e('Inline Forms', 'sandbox'); ?></a>
            <a href="?page=engage-bay-lp-builder&tab=landing-pages" class="nav-tab <?php if ($active_tab == 'landing-pages') {
            echo 'nav-tab-active';
        } ?>"><?php _e('Landing Pages', 'sandbox'); ?></a>
            <a href="?page=engage-bay-lp-builder&tab=email-templates" class="nav-tab <?php if ($active_tab == 'email-templates') {
            echo 'nav-tab-active';
        } ?>"><?php _e('Email Templates', 'sandbox'); ?></a>
             <a href="?page=engage-bay-lp-builder&tab=settings" class="nav-tab <?php if ($active_tab == 'settings') {
            echo 'nav-tab-active';
        } ?>"><?php _e('Settings', 'sandbox'); ?></a>
              <a href="?page=engage-bay-lp-builder&tab=support" class="nav-tab <?php if ($active_tab == 'support') {
            echo 'nav-tab-active';
    ...
    do_settings_sections('engage-bay-lp-builder'); ?>          
    </div>
    <?php
}

add_action('admin_menu', 'engagebay_lp_builder_add_new_menu_items');

add_action('admin_init', 'engagebay_lp_builder_display_options');
function engagebay_lp_builder_display_options()
{
    add_settings_section('home', '', 'engagebay_lp_builder_display_header_options_content', 'engage-bay-lp-builder');

    //here we display the sections and options in the settings page based on the active tab
    if (isset($_GET['tab'])) {
        if ($_GET['tab'] == 'home') {
            add_settings_section('header_logo', '', 'engagebay_lp_builder_dashboard_page', 'engage-bay-lp-builder', 'home');
        } elseif ($_GET['tab'] == 'web-pop') {
            add_settings_section('advertising_code', '', 'engagebay_lp_builder_webpoups_page', 'engage-bay-lp-builder', 'home');
        } elseif ($_GET['tab'] == 'forms') {
            add_settings_section('advertising_code', '', 'engagebay_lp_builder_forms_page', 'engage-bay-lp-builder', 'home');
        } elseif ($_GET['tab'] == 'landing-pages') {
            add_settings_section('advertising_code', '', 'engagebay_lp_builder_landing_page', 'engage-bay-lp-builder', 'home');
        } elseif ($_GET['tab'] == 'email-templates') {
            add_settings_section('advertising_code', '', 'engagebay_lp_builder_email_page', 'engage-bay-lp-builder', 'home');
        } elseif ($_GET['tab'] == 'support') {
            add_settings_section('advertising_code', '', 'engagebay_lp_builder_support_page', 'engage-bay-lp-builder', 'home');
        } else {
            if ($_GET['tab'] == 'register') {
                add_settings_section('advertising_code', '', 'engagebay_lp_builder_registration_page', 'engage-bay-lp-builder', 'home');
            } else {
                add_settings_section('advertising_code', '', 'engagebay_lp_builder_settings_page', 'engage-bay-lp-builder', 'home');
            }
        }
    } else {
        add_settings_section('header_logo', '', 'engagebay_lp_builder_dashboard_page', 'engage-bay-lp-builder', 'home');
...
function engagebay_lp_builder_display_header_options_content()
{
    //echo "The header of the theme";
}

function engagebay_lp_builder_dashboard_page()
{
    engagebay_lp_builder_css();
    $rest_api = (sanitize_text_field(get_option('engagebay_lp_builder_rest_api')));
    $domain = (sanitize_text_field(get_option('engagebay_lp_builder_domain')));
    $email = (sanitize_email(get_option('engagebay_lp_builder_email')));
    if (empty($email)) {
        wp_redirect(engagebay_lp_builder_settings_page());
    } else {
        ?>
<div id="features">
<a href="admin.php?page=engage-bay-lp-builder&tab=web-pop" id="boxm" >
<div class="box">
...
<a href="admin.php?page=engage-bay-lp-builder&tab=forms" id="boxm">
<div class="box">
  <div class="right stripline">
    <div class="header">
...
<a href="admin.php?page=engage-bay-lp-builder&tab=landing-pages" id="boxm"> 
   <div class="box">
   <div class="right stripline">
...
<a href="admin.php?page=engage-bay-lp-builder&tab=email-templates" id="boxm">
  <div class="box">
   <div class="right stripline">
...
function engagebay_lp_builder_forms_page()
{
    $rest_api = (sanitize_text_field(get_option('engagebay_lp_builder_rest_api')));
    $domain = (sanitize_text_field(get_option('engagebay_lp_builder_domain')));
    $email = (sanitize_email(get_option('engagebay_lp_builder_email')));
    $password = (sanitize_text_field(get_option('engagebay_lp_builder_password')));
    $js_api = (sanitize_text_field(get_option('engagebay_lp_builder_js_api')));
    add_thickbox();
    if (empty($email)) {
        wp_redirect(engagebay_lp_builder_settings_page());
    } else {
                                      ...
                                      <div class="">
                                        <div class="">
                                            ...
                                            <form action="https://<?php echo $domain; ?>.engagebay.com/user-login#forms" method="post" target="_blank">
                                              <?php wp_nonce_field('engagebay_lp_builder_nonce_action', 'engagebay_lp_builder_nonce_field'); ?>
...
function engagebay_lp_builder_landing_page()
{
    $rest_api = (sanitize_text_field(get_option('engagebay_lp_builder_rest_api')));
    $domain = (sanitize_text_field(get_option('engagebay_lp_builder_domain')));
    $email = (sanitize_email(get_option('engagebay_lp_builder_email')));
...
function engagebay_lp_builder_webpoups_page()
{
    $rest_api = (sanitize_text_field(get_option('engagebay_lp_builder_rest_api')));
    $domain = (sanitize_text_field(get_option('engagebay_lp_builder_domain')));
    $email = (sanitize_email(get_option('engagebay_lp_builder_email')));
...
function engagebay_lp_builder_email_page()
{
    $rest_api = (sanitize_text_field(get_option('engagebay_lp_builder_rest_api')));
    $domain = (sanitize_text_field(get_option('engagebay_lp_builder_domain')));
    $email = (sanitize_email(get_option('engagebay_lp_builder_email')));
...
function engagebay_lp_builder_settings_page()
{
    if (!current_user_can('install_plugins') && !current_user_can('install_plugins')) {
        return;
    }
    engagebay_lp_builder_css();
    $deprecated = null;
    ...
    if (isset($email) && check_admin_referer('engagebay_lp_builder_nonce_action', 'engagebay_lp_builder_nonce_field')) {
        $email = sanitize_email($_POST['email']);
        ...
        if ($domain == '' && $rest_api == '') {
            ...
            if (get_option('engagebay_lp_builder_password') !== false) {
                update_option('engagebay_lp_builder_password', $password);
            } else {
                add_option('engagebay_lp_builder_password', $password, $deprecated, $autoload);
            }

            if (get_option('engagebay_lp_builder_rest_api') !== false) {
                update_option('engagebay_lp_builder_rest_api', $rest_api);
            } else {
                add_option('engagebay_lp_builder_rest_api', $rest_api, $deprecated, $autoload);
            }

            if (get_option('engagebay_lp_builder_domain') !== false) {
                update_option('engagebay_lp_builder_domain', $domain);
            } else {
                add_option('engagebay_lp_builder_domain', $domain, $deprecated, $autoload);
            }

            if (get_option('engagebay_lp_builder_js_api') !== false) {
                update_option('engagebay_lp_builder_js_api', $js_api);
            } else {
                add_option('engagebay_lp_builder_js_api', $js_api, $deprecated, $autoload);
            }

            if (get_option('engagebay_lp_builder_email') !== false) {
                update_option('engagebay_lp_builder_email', $email);
            } else {
                add_option('engagebay_lp_builder_email', $email, $deprecated, $autoload);
            }
            ...
            <div class="panel-body">
              <form name="loginForm" id="loginForm" class="" action="" method="post">
                ...
                <div class="form-group">
                  <div class="">
                    <input class="form-control" type="email" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,10}$" name="email" autocomplete="off" placeholder="Work Email" value="<?php echo $email; ?>" required>
                  </div>
                </div>
                <div class="form-group">
                  <div class="">
                ...
                <div class="form-group">
                  <div class="xs-pt-20 md-pt-20">
                    ...
                    <div class="btn btn-success btn-xl gsuite-btn-div">
                      ...
                      <a class="gsuite-btn-text" href="https://app.engagebay.com/oauth?wordpressPlugin=yes&wordpressPluginURL=<?php echo admin_url('admin.php?page=engage-bay-lp-builder');?>">SIGN IN WITH G SUITE</a>
                    </div>
                  </div>
                </div>
             ...
             <?php
engagebay_lp_builder_custom_js(); ?>
      </div>
          </div>
         <?php if ($email == '') {
            ?>
<div class="splash-footer">
  ...
  <div class="alert text-center">
              <div>
                  ...
                  <div>Don't have an account? <a href="https://app.engagebay.com/signup" target="_blank"><?php _e('Sign Up', 'sandbox'); ?></a></div>
               </div> 
   </div>
...
<!--      <div><a href="admin.php?page=engage-bay-lp-builder&tab=settings&edit=domain">Click to Edit details</a></div>-->
    </div>
    </form>
  </div>
 <!-- <div class="col-md-6">
...
<?php
        ...
        wp_redirect(engagebay_lp_builder_options_page());
    } ?>
  </div>
</div>
</div>
...
function engagebay_lp_builder_registration_page()
{
    if (!current_user_can('install_plugins') && !current_user_can('install_plugins')) {
        return;
    }
    ...
    $autoload = 'no';
    $email = sanitize_email($_POST['email']);
    if ($email && check_admin_referer('engagebay_lp_builder_nonce_action', 'engagebay_lp_builder_nonce_field')) {
        $engagebay_lp_builder_array['email'] = sanitize_email($_POST['email']);
        $engagebay_lp_builder_array['name'] = sanitize_text_field($_POST['name']);
        $engagebay_lp_builder_array['website'] = sanitize_text_field($_POST['website']);
        $engagebay_lp_builder_array['password'] = sanitize_text_field($_POST['password']);
        $engagebay_lp_builder_array['command'] = sanitize_text_field($_POST['command']);
        $engagebay_lp_builder_array['timeZoneId'] = sanitize_text_field($_POST['timeZoneId']);
        $engagebay_lp_builder_array['source'] = 'WORDPRESS';
        //$engagebay_lp_builder_json = json_encode($engagebay_lp_builder_array);
        ...
        'httpversion' => '1.0',
        'blocking' => true,
        'body' => $engagebay_lp_builder_array,
        'headers' => array('ebwhitelist' => true)
          )
    );
        $result = wp_remote_retrieve_body($request);
      ...
      <div class="main-content container-fluid">
        ...
        ?>
          ...
          <div class="splash-footer alert">
            Already have an account? <a href="?page=engage-bay-lp-builder"  ><?php _e('Sign In', 'sandbox'); ?></a>
          </div>
           <?php
    } else {
        ?>
...
function engagebay_lp_builder_support_page()
{
    ?>
<br/>
...
function engagebay_lp_builder_footer()
{
    $rest_api = (sanitize_text_field(get_option('engagebay_lp_builder_rest_api')));
    $domain = (sanitize_text_field(get_option('engagebay_lp_builder_domain')));
    $email = (sanitize_email(get_option('engagebay_lp_builder_email')));
...
add_action('wp_footer', 'engagebay_lp_builder_footer');

// plugin deactivation
register_deactivation_hook(__FILE__, 'engagebay_lp_builder_deactivate');
function engagebay_lp_builder_deactivate()
{
    ...
    delete_option('engagebay_lp_builder_js_api');
    delete_option('engagebay_lp_builder_password');
}

add_action('admin_notices', 'engagebay_lp_builder_admin_notices');
function engagebay_lp_builder_admin_notices()
{
    $engagebay_lp_builder_email = get_option('engagebay_lp_builder_email');
    if ($engagebay_lp_builder_email == '') {
        if (isset($_GET['page']) && $_GET['page'] != 'engage-bay-lp-builder') {
            echo "<div class='updated'><p>Almost done! <a href='admin.php?page=engage-bay-lp-builder'>Enter your engagebay details </a> and you'll be ready to rock.</p></div>";
        }
    }
}

add_action('wp_dashboard_setup', 'engagebay_lp_builder_dashboard_setup_function');

function engagebay_lp_builder_dashboard_setup_function()
{
    add_meta_box('engagebay_lp_builder_dashboard_widget', 'EngageBay - Free Marketing Plugin', 'engagebay_lp_builder_dashboard_widget_function', 'dashboard', 'side', 'high');
}

function engagebay_lp_builder_dashboard_widget_function()
{
    wp_enqueue_style('engagebay-forms', plugins_url('/css/page.css', __FILE__)); ?>
...
add_action('load-post.php', 'engagebay_lp_builder_page_post_meta_boxes_setup');
add_action('load-post-new.php', 'engagebay_lp_builder_page_add_post_meta_boxes');
add_action('save_post', 'engagebay_lp_builder_page_save_postdata');

function engagebay_lp_builder_page_post_meta_boxes_setup()
{
    add_action('add_meta_boxes', 'engagebay_lp_builder_page_add_post_meta_boxes');
}

function engagebay_lp_builder_page_add_post_meta_boxes()
{
    add_meta_box('engagebay_lp_builder_page_section', __('Engagebay Section', 'engagebay_lp_builder_page_metabox'), 'engagebay_lp_builder_page_post_box', 'page', 'advanced', 'high');
}

function engagebay_lp_builder_page_save_postdata($post_id)
{
    if (isset($_POST['engagebay_lp_builder_landing_page'])) {
        $landing_page = sanitize_text_field($_POST['engagebay_lp_builder_landing_page']);
        update_post_meta($post_id, 'engagebay_lp_builder_landing_page', $landing_page);
    }
}

function engagebay_lp_builder_page_post_box($post)
{
    echo '<style>';
    ...
    if (isset($result) && count($result) > 0) {
        ...
        echo '</label> ';
        $landing_page = get_post_meta($post->ID, 'engagebay_lp_builder_landing_page', true);
        echo '<select id="engagebay_lp_builder_landing_page" autocomplete="off" name="engagebay_lp_builder_landing_page">';
        ...
        echo '<li>';
        echo '<label for="engagebay_lp_builder_landing_page">'.__('Landing Page :');
        echo '</label> ';
        echo '<select id="engagebay_lp_builder_landing_page" autocomplete="off" name="engagebay_lp_builder_landing_page" disabled>';
...
add_action('admin_head', 'engagebay_lp_builder_button');
function engagebay_lp_builder_button()
{
    global $typenow;
    ...
    if (get_user_option('rich_editing') == 'true') {
        add_filter('mce_external_plugins', 'engagebay_lp_builder_add_tinymce_plugin');
        add_filter('mce_buttons', 'engagebay_lp_builder_register_button');
    }
}

function engagebay_lp_builder_add_tinymce_plugin($plugin_array)
{
    $plugin_array['engagebay_lp_builder_button'] = plugins_url('/js/engagebay.js', __FILE__); // CHANGE THE BUTTON SCRIPT HERE
    return $plugin_array;
}

function engagebay_lp_builder_register_button($buttons)
{
    array_push($buttons, 'engagebay_lp_builder_button');

    return $buttons;
}

add_shortcode('engagebay_lp_builder', 'engagebay_lp_builder');
function engagebay_lp_builder($atts, $content, $tag)
{
    if (isset($atts['id'])) {
        $rest_api = (sanitize_text_field(get_option('engagebay_lp_builder_rest_api')));
        $domain = (sanitize_text_field(get_option('engagebay_lp_builder_domain')));
...
add_action('init', 'engagebay_lp_builder_list_enagebay_form', 5, 0);
function engagebay_lp_builder_list_enagebay_form()
{
    if (isset($_GET['engagebay_lp_builder_list_form']) == 1) {
        $rest_api = (sanitize_text_field(get_option('engagebay_lp_builder_rest_api')));
        $domain = (sanitize_text_field(get_option('engagebay_lp_builder_domain')));
        $email = (sanitize_email(get_option('engagebay_lp_builder_email')));
        if ($domain != '' && $email != '' && $rest_api != '') {
...
add_shortcode('engagebay_lp_builderform', 'engagebay_lp_builderform');
function engagebay_lp_builderform($atts, $content, $tag)
{
    if (isset($atts['id'])) {
        $rest_api = (sanitize_text_field(get_option('engagebay_lp_builder_rest_api')));
        $domain = (sanitize_text_field(get_option('engagebay_lp_builder_domain')));
...
add_action('wp_enqueue_scripts', 'engagebay_lp_builder_refresh');

function engagebay_lp_builder_refresh()
{
    if (isset($_POST['name'])) {
        wp_enqueue_script('engagebay_lp_builder_refresh', plugins_url('js/refresh.js', __FILE__));
    }
}

add_action('admin_enqueue_scripts', 'engagebay_lp_builder_customjs');
function engagebay_lp_builder_customjs()
{
